...
(defn format-state [state]
  (case state
    ...
    (<< [:svg {:class $state-icon :xmlns "http://www.w3.org/2000/svg" :xmlns:xlink "http://www.w3.org/1999/xlink" :version "1.1" :id "Layer_1" :viewBox "0 0 512 512" :xml:space "preserve"}
...
(defn ui-root [state]
  (<< [:div
       [:h1 {:class (css :p-4 :font-bold :text-lg)} "Fragments"]
       ...
       [:div {:class (css :flex)}
        [:table {:class (css :flex-1)}
         ...
         [:tbody
          (sg/simple-seq state ui-row)]]

        [:div {:class (css :flex-1)}
         [:div {:class (css :p-4)}
         "This animation should show stutter if the 16ms budget is used. Sorry, too lazy to find something pretty. This is just what I had at hand. Even with 20x slowdown on my m4 pro mac mini it hardly stutters."]
...
(defn init []
  ...
  (js/setInterval
    (fn []
      ...
      (-> (js/Promise.resolve)
          ;; first the update the data, fast
          (.then (fn [] (swap! state-ref update-random)))
          ;; then we generate the hiccup, fast-ish
          (.then (fn [] (ui-root @state-ref)))
          ;; when we apply it to update the actual DOM, slowest
          (.then (fn [hiccup] (sg/render app js/document.body hiccup)))))
    100))
